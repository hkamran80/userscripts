...
document.querySelectorAll("li.work, li.bookmark").forEach(function(work) {
    ...
    let title = work.querySelector(".heading > a").textContent,
        ...
        summary = html_escape(work.querySelector("blockquote.summary").textContent).trim().replace(/\.(\S)/g, ". $1"),
        id = _match[1],
        filename = title.replace(/[^\w _-]+/g, "").replace(/ +/g, " ").replace(/^(.{24}[\w.]*).*/, "$1"),
        download_url = base_download_url + id + "/" + filename;
    
    let results = '<p><span class="downloadtitle" title="' + summary + '">' + title + '</span> by ' + author + '  <span class="downloadlinks"><a href="' + download_url + '.mobi">mobi</a> <a href="' + download_url + '.epub">epub</a> <a href="' + download_url + '.pdf">pdf</a> <a href="' + download_url + '.html">html</a></span></p>';
    epub_links.push(download_url + ".epub");
    results_div.innerHTML += results;
});

results_div.innerHTML += "<br /><br /><textarea readonly>" + epub_links.join("\n") + "</textarea>";
...
button.appendChild(document.createTextNode('Download Series ' + String.fromCharCode(0x2193)));
var buttonParent = document.createElement('li');
buttonParent.className = 'chapter';
buttonParent.appendChild(button);
var bookmark_series_parent = document.querySelector("a.bookmark_form_placement_open").parentElement;
